...
export function setup() {
  const loginUrl = 'https://photoalbum-backend-photoalbum-f041om.apps.okd.fured.cloud.bme.hu/api/login/';
  ...
  const authToken = res.json('access') || res.json('token');
...
export default function (data) {
  const url = 'https://photoalbum-backend-photoalbum-f041om.apps.okd.fured.cloud.bme.hu/api/photos/';

  const formData = {
    name: 'Load Test Photo',
    image: http.file(JPEG_BYTES, 'test-image.jpg', 'image/jpeg'),
  };
  ...
  if (res.status !== 201) {
    console.log(`Upload Failed! Status: ${res.status} Body: ${res.body}`);
  }
